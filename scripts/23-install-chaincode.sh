...
for i in ${!ORG_NAMES[@]}; do
    ...
    # Execute the install command via SSH + docker exec
    INSTALL_OUTPUT=$(ssh fabricadmin@$ORG_IP "docker exec $CLI_CONTAINER bash -c '$INSTALL_CMD'" 2>&1)
    INSTALL_EXIT_CODE=$?

    echo "$INSTALL_OUTPUT" >> $LOG_FILE # Log output regardless of success/failure

    if [ $INSTALL_EXIT_CODE -ne 0 ]; then
        log "⛔ Error: Failed to install chaincode for $ORG_NAME."
    ...
    echo "$ORG_NAME_PACKAGE_ID=$PACKAGE_ID" >> "$LOG_DIR/chaincode_package_ids.txt"
